...
for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
    ...
    # 핵심 진료과가 위로 가도록 표현 예 : 김안과의원 핵심진료과 안과 idx 0번으로 셋팅
    search_idx = [i for i in range(len(parts)) if parts[i] in hosp_name]
    if len(search_idx) != 0:
        if search_idx[0] != 0:
            temp = parts[search_idx[0]]
            parts.remove(temp)
            parts.insert(0, temp)
...
new_df.to_csv(folder_path + 'T_RCMN_HOSP_MDXM_SUBJ_211019_ver2-utf-8-sig', encoding='utf-8-sig')
